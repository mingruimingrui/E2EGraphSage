...
https://github.com/pytorch/pytorch/blob/v1.1.0/torch/utils/data/dataloader.py
Except this dataloader makes use of datasets that takes in list for __getitem__
"""
...
from ._utils.worker import _worker_loop
...
class DataLoader2(object):
    """
    This is basically a clone of torch.utils.data.DataLoader
    except that it inputs a list for the __getitem__function
    """
    ...
    def __setattr__(self, attr, val):
        if self.__initialized and attr in {
            ...
            raise ValueError((
                '{} attribute should not be set after {} is initialized'
            ).format(attr, self.__class__.__name__))

        super(DataLoader2, self).__setattr__(attr, val)

    def __iter__(self):
        return _DataLoaderIter2(self)

    def __len__(self):
        return len(self.batch_sampler)


class _DataLoaderIter2(object):
    """
    Similar to _DataLoaderIter but for DataLoader2
    """
